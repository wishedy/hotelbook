...
var domain = {
    getRootPath: function () {
        //获取当前网址，如： http://localhost:8081:8080/ems/Pages/Basic/Person.jsp
        ...
        //获取主机地址，如： http://localhost:8081:8080
        var localhost:8081Path = curWwwPath.substring(0, pos);
        //获取带"/"的项目名，如：/ems
        var projectName = pathName.substring(0, pathName.substr(0).indexOf('/') );
        if (projectName == "/yiding_static") {
            return localhost:8081Path;
        } else {
            return (localhost:8081Path + projectName);
        }
    },
    charset: "utf-8",
    get: function () {
